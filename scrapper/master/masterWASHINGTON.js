	...
	function getURLwashington()
	{
	  ...
	  request({
		...
		$("body a").each(function(i) {
		  ...
		  {*/
          if (myhref !== undefined){
		  if (myhref.substring(0,1) === "/" && myhref.substring(0,7) !== "/posttv")
		  {
			oldhref = myhref;
			myhref = "http://www.thewashington.com"+myhref;
			tarray.push(myhref);
			//console.log(myfile);
			console.log(myhref);
			i++;
	...
	function getArticlewashington(array){
	  ...
	  while (j<array.length-1)
	  {
		...
		fs.appendFile("Articles/WASHINGTON/WASHINGTON_"+myfile, array[k]+"\n", function (err) {});
		  ...
		  $("div div div div h1").each(function(element) {
			//console.log(myfile);
			var paragraph = $(this);
          ...
          $("#content #article-leaf-page div div h3").each(function(element) {
			//console.log(myfile);
			var paragraph = $(this);
			var text = paragraph.text()+"\n";
			//console.log(text);
